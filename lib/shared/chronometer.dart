...
enum ChronometerExecute { start, stop, reset, lap }

/// StopWatchMode
enum ChronometerMode { countUp, countDown }

/// StopWatchTimer
class Chronometer {
  Chronometer({
    this.isLapHours = true,
    this.mode = ChronometerMode.countUp,
    ...
    _executeController.listen((value) {
      switch (value) {
        case ChronometerExecute.start:
          _start();
          break;
        case ChronometerExecute.stop:
          _stop();
          break;
        case ChronometerExecute.reset:
          _reset();
          break;
        case ChronometerExecute.lap:
          _lap();
          break;
      }
    });
  ...
  final BehaviorSubject<List<ChronometerRecord>> _recordsController =
  BehaviorSubject<List<ChronometerRecord>>.seeded([]);
  ValueStream<List<ChronometerRecord>> get records => _recordsController;

  final PublishSubject<ChronometerExecute> _executeController =
  PublishSubject<ChronometerExecute>();
  Stream<ChronometerExecute> get execute => _executeController;
  Sink<ChronometerExecute> get onExecute => _executeController.sink;
  ...
  int? _second;
  int? _minute;
  List<ChronometerRecord> _records = [];
  ...
  void startFromNewPreset(int newPresetMillis) {
    _presetTime = newPresetMillis;
    _initialPresetTime = newPresetMillis;
    // _elapsedTime.add(_presetTime);
    _reset();
    _start();
  }
  ...
  void _lap() {
    if (isRunning) {
      ...
      _records.add(ChronometerRecord(
        rawValue: rawValue,
